...
function MemberMobileGreetingCard({
  row,
  onView,
}: {
  row: GreetingFormRow;
...
/** Member-scoped 인사장관리 list (same layout as admin on desktop). */
export function MemberGreetingMng({
  reloadToken = 0,
}: {
  /** Bump after linked greeting save so list refreshes. */
  reloadToken?: number;
}) {
    ...
    <div className="space-y-3">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <section
            className={cn(
              "hidden min-w-0 rounded-lg border border-line bg-panel p-3.5 min-[1040px]:block",
            )}
          >
            <Table
              caption="내 인사장 목록"
...
export default MemberGreetingMng;
